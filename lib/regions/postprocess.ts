...
import { insetQuad, warpQuadRgb } from "../vision/warp-rgb.ts";
import { boundsOf, quadPoints, type Mask, type Quad, type Rect, type Rgb } from "../vision/types.ts";
...
export function renderPhotoCrop(
  ...
  const sourceShort = Math.min(measured.width, measured.height);
  ...
  const image = warpQuadRgb(source, inset, width, height);
