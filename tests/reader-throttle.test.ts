...
import { admitReaderScan, resetReaderThrottle, scansPerMinute, DEFAULT_SCANS_PER_MINUTE } from "../lib/reader/throttle.ts";
...
test("the environment value parses, and malformed values fall back to the default — never to unlimited", () => {
  ...
  assert.equal(scansPerMinute({ FORMLINK_TEXT_MAX_SCANS_PER_MINUTE: "25" }), 25);
  assert.equal(scansPerMinute({ FORMLINK_TEXT_MAX_SCANS_PER_MINUTE: "0" }), 0);
  assert.equal(scansPerMinute({ FORMLINK_TEXT_MAX_SCANS_PER_MINUTE: "lots" }), DEFAULT_SCANS_PER_MINUTE);
  assert.equal(scansPerMinute({ FORMLINK_TEXT_MAX_SCANS_PER_MINUTE: "-5" }), DEFAULT_SCANS_PER_MINUTE);
  assert.equal(scansPerMinute({ FORMLINK_TEXT_MAX_SCANS_PER_MINUTE: "Infinity" }), DEFAULT_SCANS_PER_MINUTE);
});
